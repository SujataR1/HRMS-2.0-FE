...
import Illustration from "../../assets/right.png"; // Adjust path as needed
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="md:w-1/2 p-8 md:p-12">
          <h2 className="text-3xl font-bold text-[#f1d245] mb-3">
            {step === 1 ? "HR Forgot Password" : "Reset Password"}
          </h2>
          <p className="text-gray-600 mb-8">
            {step === 1
              ? "Enter the email address associated with your HR account."
          ...
          {step === 1 ? (
            <form onSubmit={handleEmailSubmit} className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`px-6 py-2 bg-[#f1d245] text-white rounded-full shadow-md hover:bg-[#e6c822] transition font-semibold ${loading ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                >
            ...
            <form onSubmit={handleResetPassword} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full py-2 bg-[#f1d245] text-white rounded-full shadow-md hover:bg-[#e6c822] transition font-semibold ${loading ? "opacity-50 cursor-not-allowed" : ""
                  }`}
              >
          ...
          {message && (
            <p className="text-yellow-600 mt-4 font-medium">{message}</p>
          )}
          {error && (
