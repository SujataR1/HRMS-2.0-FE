...
// New function to check employee presence - FIXED HEADERS
const checkEmployeePresence = async () => {
  ...
  try {
    const token = localStorage.getItem('token');
    const adminToken = localStorage.getItem('admin_token'); // Check if admin_token exists
    
    // Use admin_token if available, otherwise use token
    ...
    const response = await fetch('https://backend.hrms.transev.site/admin/check-employee-presence', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
