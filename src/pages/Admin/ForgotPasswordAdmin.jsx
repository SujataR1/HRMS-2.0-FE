...
import Illustration from "../../assets/right.png"; // Adjust if needed
...
const ForgotPasswordAdmin = () => {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://backend.hrms.transev.site/admin/request-password-reset",
        { email }
      );
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://backend.hrms.transev.site/admin/reset-password",
        {
          email,
          otp,
          newPassword,
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="md:w-1/2 p-8 md:p-12">
          <h2 className="text-3xl font-bold text-[#f1d245] mb-3">
            {step === 1 ? "Admin Forgot Password" : "Reset Password"}
          </h2>
          ...
          {/* Feedback Messages */}
          {message && <p className="text-yellow-600 mt-4 font-medium">{message}</p>}
