...
const AllEmployees = () => {
  ...
  useEffect(() => {
    ...
    const performSearch = async () => {
      ...
      try {
        const res = await fetch("https://backend.hrms.transev.site/admin/search-employees", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("admin_token")}`,
        ...
        const matched = data.data.map((match) => {
          ...
          return {
            ...full,
            matchedField: match.propertyMatched,
            matchedValue: match.matchFound,
  ...
  const listToRender = search.trim() ? searchResults : employees;
        ...
        <div className="max-w-7xl mx-auto bg-white rounded-3xl shadow-xl border border-yellow-300 p-8">
          ...
          {loading || searching ? (
            ...
            <div className="text-center mt-16 space-y-4" role="alert">
              ...
              <p className="text-gray-600 italic text-xl">No employees found.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
              {listToRender.map((emp) => (
                ...
                >
                  ...
                  <div className="mt-14 space-y-2">
                    <h3 className="text-2xl font-extrabold text-yellow-700 truncate" title={emp.name}>
                      {emp.name}
                    </h3>
                    <p className="text-yellow-800 font-semibold truncate" title={`Employee ID: ${emp.employeeId}`}>
                      ID: {emp.employeeId}
                    </p>
                    ...
                    <p className="text-gray-700 truncate" title={emp.employeeDetails?.designation || "N/A"}>
                      <span className="font-semibold">Designation:</span>{" "}
                      {emp.employeeDetails?.designation || "N/A"}
                    </p>

                    <p className="text-gray-700 truncate flex items-center justify-between" title={emp.employeeDetails?.department || "N/A"}>
                      <span className="font-semibold">Department:</span>{" "}
                      <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${getBadgeColor(emp.employeeDetails?.department)}`}>
                        {emp.employeeDetails?.department || "N/A"}
                      </span>
                    </p>
                  </div>
