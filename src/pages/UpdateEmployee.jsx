...
const UpdateEmployee = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage(null);
    try {
      const res = await fetch('http://localhost:9000/admin/update-employee-details', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('admin_token')}`,
      ...
      <main className="flex-1 bg-gray-50 p-6 overflow-auto">
        <div className="max-w-4xl mx-auto bg-white p-8 rounded shadow border border-gray-200">
          ...
          {loading ? (
            ...
            <div
              className={`text-center mb-4 px-4 py-2 rounded ${message.type === 'error' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                }`}
            >
              {message.text}
            </div>
